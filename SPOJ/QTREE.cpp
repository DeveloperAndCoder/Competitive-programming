...
int baseArray[N], ptr;
int chainNo, chainInd[N], chainHead[N], posInBase[N], ulta[N];
 ...
 */
void update_tree(int cur, int s, int e, int x, int val = 0)
{
    if(s > x || e <= x) return;
    if(s == x && s == e)
    {
        if(color[x])    st[cur] = ulta[cur];
        else st[cur] = N+1;
        return;
    }
    int c1 = (cur<<1)+1, c2 = c1 + 1, m = (s+e)>>1;
...
int query_up(int u, int v = 0)
{
    ...
    int uchain, vchain = chainInd[v], ans = N+1;
    ...
    while(1)
    {
        ...
        if(uchain == vchain)
        {
            ...
            query_tree(0, 0, ptr-1, posInBase[v]+1, posInBase[u]+1);
            // Above is call to segment tree query function
            ans = min(ans,qt[0]); // Update answer
            break;
        }
        query_tree(0, 0, ptr-1, posInBase[chainHead[uchain]], posInBase[u]+1);
...
int main()
{
    ...
    while(t--)
    {
        ...
        HLD(root, N+1,-1); // We decomposed the tree and created baseArray
